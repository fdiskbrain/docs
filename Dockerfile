...
RUN wget https://oss.sonatype.org/content/repositories/releases/org/mybatis/mybatis-migrations/3.3.5/mybatis-migrations-3.3.5-bundle.zip &&\
    ...
    [ $(md5sum mybatis-migrations-3.3.5-bundle.zip | awk '{print $1}') == $(cat mybatis-migrations-3.3.5-bundle.zip.md5) ]  &&\
...
RUN mkdir /datadog &&\
    ...
    [ $(md5sum dd-java-agent-0.33.0.jar | awk '{print $1}') == $(cat dd-java-agent-0.33.0.jar.md5) ] &&\
